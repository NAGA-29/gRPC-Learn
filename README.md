...
| [Step 11](./step11-advanced-testing/) | 応用編 — bufconn テスト / リトライポリシー | Go |
...
# buf のインストール（Linux）
sudo curl -sSL https://github.com/bufbuild/buf/releases/latest/download/buf-Linux-x86_64 -o /usr/local/bin/buf
...
`bash scripts/gen.sh` を実行すると、`buf.gen.go.yaml`（Go 用）と `buf.gen.ts.yaml`（TypeScript 用）の設定に従って、各 step のディレクトリにコードが生成されます。
...
> 生成されたコードは編集しません。`.proto` を変更したら必ず `bash scripts/gen.sh` で再生成します。
...
1. `.proto` を編集する
2. `bash scripts/gen.sh`（および必要なら `bash scripts/gen-python.sh`）でスタブを再生成
...
```
grpc-learning-lab/
├── buf.yaml              # buf v2 ワークスペース設定（proto/ を単一モジュールとして管理）
├── buf.gen.go.yaml       # Go コード生成設定
├── buf.gen.ts.yaml       # TypeScript コード生成設定
├── scripts/
│   ├── gen.sh            # 全ステップの Go / TS スタブ一括生成スクリプト
...
`bash scripts/gen.sh` を実行することで Go スタブおよび TypeScript クライアントコードが生成されます。
...
> **注意:** `bash scripts/gen.sh` を実行しないまま `go run .` や `npm start` を行うと、
